...
def path_from_to(root, target):
    ...
    while (len(queue) > 0):
        ...
        ##comapre total of cost and heuristic value, choose lesser
...
def findParentInsertChild(rootNode,currentIndex,parentIndex , totalcost):
    if rootNode is None:
        return
    findParentInsertChild(rootNode.left,currentIndex,parentIndex ,totalcost)
    
    if rootNode.data == parentIndex:
        if rootNode.left is None:
            rootNode.left = TreeNode(currentIndex , totalcost)
        else:
            rootNode.right = TreeNode(currentIndex , totalcost)
    
    findParentInsertChild(rootNode.right,currentIndex,parentIndex , totalcost)  
    
    
## declare TreeNode add cost to store total of cost and heuristic value
class TreeNode:
    def __init__(self, data, cost):
        self.left = None
        self.right = None
        self.data = data
        self.cost = cost #f = g+ h 
...
class AStarPlanner:
    ...
    def planning(self, sx, sy, gx, gy):
        ...
        while 1:
            ...
            ##calculate total of cost and heuristic value f = g+h
            key=lambda o: open_set[o].cost + self.calc_heuristic(goal_node,open_set[o])
